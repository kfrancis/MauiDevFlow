...
- Target frameworks: `net10.0-android`, `net10.0-ios`, `net10.0-maccatalyst` for MAUI libraries; `net9.0` for the CLI tool
- Minimal comments — only clarify non-obvious logic
- Use `record` types for data transfer objects (e.g., `FileLogEntry`)
- Platform-specific code uses `#if ANDROID`, `#if IOS || MACCATALYST`, etc.
