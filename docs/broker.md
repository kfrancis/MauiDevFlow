...
MauiDevFlow includes a **broker daemon** that coordinates port assignment and agent
...
When a MAUI app starts with `AddMauiDevFlowAgent()`:

1. The agent reads its **project identity** from assembly metadata injected at build time
   (`MauiDevFlowProject` = absolute path to `.csproj`, `MauiDevFlowTfm` = e.g.
   `net10.0-maccatalyst`).
...
This ensures no collisions even with non-MauiDevFlow processes using ports in the range.
...
2. Broker unavailable            → read MauiDevFlowPort from assembly metadata
...
- **Custom port in code?** If `AddMauiDevFlowAgent(o => o.Port = XXXX)` sets a
